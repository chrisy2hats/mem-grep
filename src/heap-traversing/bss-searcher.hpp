...
class BssSearcher {
  public:
  [[nodiscard]] std::vector<RemoteHeapPointer> FindHeapPointers(const MapsEntry &heap) const;
  ...
  BssSearcher(const MapsEntry &bss_metadata, const pid_t &pid, size_t max_heap_obj);

  private:
  [[nodiscard]] inline bool AddressIsOnHeap(
		  const void *address, const MapsEntry &heap_metadata) const;
  const pid_t pid_;
  const size_t max_heap_obj_;
  const MapsEntry bss_metadata_;
};
#endif