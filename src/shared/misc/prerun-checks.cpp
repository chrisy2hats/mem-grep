...
// To be able to read memory from another process either the kernel has to have been told to allow
// this (via ptrace_scope) Or we need to be running as root Or both.
[[nodiscard]] bool CanRun(void) {
  ...
  if (sizeof(void *) != sizeof(size_t)) {
    cout << "Program running on unusual architecture where a pointer isn't as large as a "
  ...
  cout << "ERROR: Yama ptrace to restricted and program not run as root. mem-grep cannot run as it "
	  "can't access other processes memory. Exiting now...\n";
